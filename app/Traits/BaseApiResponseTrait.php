...
trait BaseApiResponseTrait
{
    ...
    {
        return $this->statusCode ?: ResponseAlias::HTTP_OK;
    }
    /**
    ...
    protected function respondWithSuccess(string $message = null, array $data = []): JsonResponse
    {
        ...
        return $this->setStatusCode(ResponseAlias::HTTP_OK)->respondWithArray($response);
    }
    ...
    {
        $statusCode = !empty($statusCode) ? $statusCode : ResponseAlias::HTTP_NOT_FOUND;
        if (is_string($errors)) {
            $errors = __($errors);
        }
