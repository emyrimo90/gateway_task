...
abstract class BaseRepository implements BaseContract
{
    ...
    public function defaultUpdateOrCreate(array $attributes, array $identifier = []): mixed
    {
        ...
        if (array_key_exists('dynamics_id', $identifier)) {
    ...
    /**
     * @param Model $model
     * @param bool $deleteDefinedRelations
     * @return bool|null
     ...
     * @return int
     */
    public function countWithFilters($filters, array $relations = []): int
    {
        $query = $this->query;
        if (!empty($relations)) {
     ...
     * @return mixed
     */
    public function sumWithFilters($column, $filters, array $relations = []): mixed
    {
        $query = $this->query;
        if (!empty($relations)) {
            $query = $query->with($relations);
    ...
    public function aggregate($function, $column): mixed
    {
        ...
        foreach ($this->model->getFilters() as $filter) {
            if (isset($filters[$filter])) {
                $withFilter = "of" . ucfirst($filter);
                $query = $this->query->$withFilter($filters[$filter]);
            }
        }
        return $query->{$function}($column);
    }
     ...
     * @return mixed
     */
    public function findBy(string $key, mixed $value, bool $fail = true, array $relations = []): mixed
    {
    ...
    public function findAll(array $fields = ['*'],
                            ...
                            string $orderBy = self::ORDER_BY,
                            string $orderDir = self::ORDER_DIR): mixed
    {
        $query = $this->query;
        if ($applyOrder) {
            $query = $query->orderBy($orderBy, $orderDir);
    ...
    public function search(
        ...
        return $this->getQueryResult($query, $applyOrder, $page, $limit,
            $orderBy, $orderDir, $customizePaginationURI, $paginationURI,
            $orderBy2, $orderDir2, $simplePaginate);
    }
     ...
     * @return mixed
     */
    public function toggleField(int $id, string $field): mixed
    {
